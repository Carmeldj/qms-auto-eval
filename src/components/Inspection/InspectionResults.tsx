...
const InspectionResults: React.FC = () => {
  ...
  const { clearInspection, completedInspection } = useInspection();
  if (!completedInspection) {
    return <div>No inspection in progress</div>;
  }
  const report = completedInspection;

  const recommendations = generateRecommendations(report.answers);
  const categories = getAllCategories();
  const realTimeStats = inspectionReportService.calculateRealTimeStats(
    report.answers
  );
  const complianceRate = inspectionReportService.getComplianceRate(
    report.answers
  );
  const complianceLevel =
    inspectionReportService.getComplianceLevel(complianceRate);
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const result = await inspectionReportService.generatePDF(report);
      ...
      await uploadAndSaveDocument(blob, fileName, {
        ...
        category: "Inspection",
        description: `Rapport d'auto-inspection pour ${report.pharmacyInfo.name} - Conformité: ${complianceRate}%`,
        author:
          user?.name ||
          user?.email ||
          report.pharmacistInfo.firstName +
            " " +
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
        ...
        <p className="text-sm text-yellow-700">
          Réponses reçues: {report.answers.length} | Conformes:{" "}
          {realTimeStats.compliant} | Non conformes:{" "}
          {realTimeStats.nonCompliant} | Non applicables:{" "}
      ...
      >
        <div className="text-center">
          ...
          <div className="text-5xl font-bold mb-2">{complianceRate}%</div>
          <p style={{ color: "#b2dfdb" }}>
            Niveau de conformité: {complianceLevel.level} •{" "}
            {realTimeStats.totalItems}/{realTimeStats.totalPossibleItems}{" "}
            éléments évalués
          </p>
          {realTimeStats.unanswered > 0 && (
            <p className="text-sm mt-2" style={{ color: "#b2dfdb" }}>
              {realTimeStats.unanswered} élément(s) non évalué(s) - considéré(s)
              comme conforme(s) par défaut
            </p>
          )}
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          {realTimeStats.nonCompliant > 0 ? (
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                ...
                <p className="text-sm text-blue-800">
                  {realTimeStats.criticalGaps > 0 &&
                    `${realTimeStats.criticalGaps} écart(s) critique(s) nécessitant une action immédiate. `}
                  {realTimeStats.majorGaps > 0 &&
                    `${realTimeStats.majorGaps} risque(s) majeur(s) à traiter en urgence. `}
                  {realTimeStats.minorGaps > 0 &&
                    `${realTimeStats.minorGaps} écart(s) mineur(s) à intégrer dans l'amélioration continue.`}
                  {realTimeStats.criticalGaps === 0 &&
                    realTimeStats.majorGaps === 0 &&
                    realTimeStats.minorGaps === 0 &&
                    realTimeStats.nonCompliant > 0 &&
                    `${realTimeStats.nonCompliant} écart(s) identifié(s) sans classification de risque.`}
                </p>
              </div>
              ...
              {report.answers.filter((a) => a.status === "non-compliant")
                .length > 6 && (
                <p className="text-sm text-gray-500 text-center">
                  Et{" "}
                  {report.answers.filter((a) => a.status === "non-compliant")
                    .length - 6}{" "}
                  autres recommandations dans le rapport PDF...
                </p>
              )}
            </div>
          ) : (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Aucune action corrective requise.</p>
        ...
        <div className="space-y-6">
          {categories.map((category) => {
            ...
            const categoryComplianceRate =
              answeredCount > 0
                ? Math.round((compliantCount / answeredCount) * 100)
                : 0;
              ...
              >
                    ...
                    {answeredCount > 0 ? (
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${
                          categoryComplianceRate >= 90
                            ? "bg-green-100 text-green-800"
                            : categoryComplianceRate >= 75
                            ? "bg-yellow-100 text-yellow-800"
                            : "bg-red-100 text-red-800"
                        }`}
                      >
                ...
                <div className="space-y-3">
                  <h5 className="font-medium text-gray-900">
                    Détail des éléments ({answeredCount} évalués sur{" "}
                    {categoryItems.length}) :
                  </h5>
                  {categoryItems.map((item) => {
                    const itemAnswer = report.answers.find(
                      (a) => a.itemId === item.id
                    );
                    const StatusIcon = getStatusIcon(
                      itemAnswer?.status || "not-evaluated"
                    );
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-600">
                            Statut:{" "}
                            {itemAnswer?.status === "compliant"
                              ? "Conforme"
                              : itemAnswer?.status === "non-compliant"
                              ? "Non conforme"
                              : itemAnswer?.status === "not-applicable"
                              ? "Non applicable"
                              ...
                              itemAnswer.gapType && (
                                <span
                                  className={`ml-2 px-2 py-1 rounded-full text-xs ${
                                    itemAnswer.gapType === "critical"
                                      ? "bg-red-100 text-red-800"
                                      : itemAnswer.gapType === "major"
                                      ? "bg-orange-100 text-orange-800"
                                      : "bg-yellow-100 text-yellow-800"
                                  }`}
                                >
                ...
                {categoryAnswers.filter((a) => a.status === "non-compliant")
                  .length > 0 && (
                  <div className="space-y-3">
                          ...
                          >
                            <div className="font-medium text-red-900">
                              ...
                              {recommendation?.title ||
                                `Corriger: ${item?.description}`}
                            </div>
                            <div className="text-sm text-red-700 mt-1">
                              {recommendation?.description ||
                                `Mettre en conformité selon: ${item?.requirement}`}
                            </div>
                            <div className="flex items-center space-x-3 mt-2 text-xs text-red-600">
                              <span>
                                Délai:{" "}
                                {recommendation?.timeframe || "1-2 semaines"}
                              </span>
                              <span>
                                Responsable:{" "}
                ...
                {compliantCount > 0 && answeredCount > 0 && (
                  <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium text-green-800">
                        {compliantCount} élément{compliantCount > 1 ? "s" : ""}{" "}
                        conforme{compliantCount > 1 ? "s" : ""} validé
                        {compliantCount > 1 ? "s" : ""}
                      </span>
                    </div>
                  </div>
                ...
                {notApplicableCount > 0 && answeredCount > 0 && (
                  <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium text-gray-800">
                        {notApplicableCount} élément
                        {notApplicableCount > 1 ? "s" : ""} non applicable
                  ...
                  <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium text-blue-800">
                        {unansweredCount} élément
                        {unansweredCount > 1 ? "s" : ""} non évalué
                        {unansweredCount > 1 ? "s" : ""}
                        (considéré{unansweredCount > 1 ? "s" : ""} conforme
                        {unansweredCount > 1 ? "s" : ""} par défaut)
                      </span>
                    </div>
                  </div>
                )}
