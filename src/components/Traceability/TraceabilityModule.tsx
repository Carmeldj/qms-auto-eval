...
import {
  ...
  getAllCategories,
} from "../../data/traceabilityTemplates";
...
const TraceabilityModule: React.FC = () => {
  ...
  const [compilationYear, setCompilationYear] = useState<number>(
    new Date().getFullYear()
  );
  const [compilationMonth, setCompilationMonth] = useState<number>(
    new Date().getMonth() + 1
  );
  ...
  const handleGenerateCompilation = async () => {
    if (!selectedTemplate || !pharmacyName) {
      alert(
        "Veuillez sélectionner un registre et saisir le nom de la pharmacie"
      );
      return;
    }

    const template = traceabilityTemplates.find(
    ...
    try {
      ...
      const result = await traceabilityService.generateMonthlyCompilation(
        template,
        compilationYear,
        compilationMonth,
        pharmacyName
      ...
      await uploadAndSaveDocument(blob, fileName, {
        title: `Compilation Mensuelle - ${template.title} - ${monthName}`,
        type: "traceability-compilation",
        category: template.category,
        description: `Compilation mensuelle du registre ${template.title} pour ${monthName}`,
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold text-gray-900">
              Compilation Mensuelle des Registres
            </h1>
            <button
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>ℹ️ Information :</strong> La compilation mensuelle
              regroupe tous les enregistrements du registre sélectionné pour le
              mois choisi dans un seul document PDF avec tableau récapitulatif.
            </p>
          </div>
        </div>
      </div>
  ...
  if (view === "form") {
    ...
    return <TraceabilityForm template={template} onCancel={handleBackToList} />;
  }
    ...
    <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-600">
              Registres officiels avec export PDF direct pour la traçabilité
              pharmaceutique
            </p>
          </div>
          <button
      ...
      <div className="bg-white rounded-xl shadow-md p-3 sm:p-4 mb-6 sm:mb-8">
        <div className="flex flex-wrap gap-2">
          <button
            ...
            style={
              selectedCategory === "all" ? { backgroundColor: "#009688" } : {}
            }
          >
            Tous les registres
          </button>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mt-8">
        ...
        <div className="space-y-2 text-blue-800">
          <p>
            1. <strong>Sélectionnez</strong> le type de registre à remplir
          </p>
          <p>
            2. <strong>Complétez</strong> le formulaire avec les informations
            requises
          </p>
          <p>
            3. <strong>Générez</strong> automatiquement le PDF officiel
          </p>
          <p>
            4. <strong>Imprimez</strong> ou archivez votre enregistrement
          </p>
        </div>
        <div className="mt-4 p-3 bg-blue-100 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>📱 Compatible mobile :</strong> Tous les formulaires sont
            optimisés pour mobile et PC
          </p>
        </div>
      </div>
    </div>
