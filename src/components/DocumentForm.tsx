...
import { mapDocumentToDto } from "../utils/documentMapper";
import { formatFileSize } from "../utils/documentMapper";
...
const DocumentForm: React.FC<DocumentFormProps> = ({ template, onCancel }) => {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const { blob, fileName } = await documentService.generatePDFBlob(
        template,
        document
      );
      ...
      const documentDto = mapDocumentToDto(document, template, {
        filePath: uploadedFilePath,
        fileSize: actualFileSize,
        fileType: "application/pdf",
        author: user?.name || user?.email || "Unknown Author",
        version: "1.0",
      ...
      // Also save locally for user download
      await documentService.generatePDF(template, document);

      // Show success message
      alert("Document généré, uploadé et sauvegardé avec succès!");
    } catch (error) {
