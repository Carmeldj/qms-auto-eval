...
export class DocumentService {
  ...
  public async generatePDF(
    ...
    try {
      ...
      if (
        template.id !== "job-description" &&
        template.id !== "quality-policy"
      ) {
        yPosition = topMargin;
        addText(
          "DOCUMENT OFFICIEL PHARMA QMS",
          18,
          true,
          "teal",
          "center",
      ...
      if (template.id === "organization-chart") {
        await this.generateOrganizationChart(
          pdf,
          document,
          // addText,
          addSection,
        ...
        await this.generateJobDescription(
          pdf,
          document,
          leftMargin,
          rightMargin,
        ...
        await this.generateQualityPolicy(
          pdf,
          document,
          leftMargin,
          rightMargin,
      ...
      if (
        template.id !== "job-description" &&
        template.id !== "organization-chart" &&
        template.id !== "quality-policy"
      ) {
        ...
        const signatureBoxWidth =
          (pageWidth - leftMargin - rightMargin - 20) / 2;
      ...
      throw new Error(
        `Erreur lors de la generation du PDF: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
      );
    }
  }
  ...
  private async generateJobDescription(
    ...
    const jobTitle = data.jobTitle
      ? this.removeAccents(data.jobTitle.toUpperCase())
      : "NON RENSEIGNE";
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(14);
    pdf.setTextColor(0, 150, 136);
    pdf.text(`FICHE DE FONCTION ${jobTitle}`, pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 6;
    ...
    pdf.setFont("helvetica", "normal");
    pdf.text(
      this.removeAccents(data.jobTitle || "Non renseigne"),
      leftMargin + 35,
      yPosition + 3.5,
    );
    ...
    pdf.setFont("helvetica", "normal");
    pdf.text(
      this.removeAccents(data.pharmacyName || "Non renseigne"),
      leftMargin + 35,
      yPosition + 7.5,
    );
    ...
    // Section 2: Mission
    addBoxedSection("MISSION PRINCIPALE", data.mission || "Non renseigne", {
      r: 0,
      g: 150,
      b: 136,
    });
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(7.5);
    const formationLines = pdf.splitTextToSize(
      this.removeAccents(data.formation || "Non renseigne"),
      columnWidth - 6,
    );
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(7.5);
    const experienceLines = pdf.splitTextToSize(
      this.removeAccents(data.experience || "Non renseigne"),
      columnWidth - 6,
    );
  ...
  private async generateQualityPolicy(
    ...
    pdf.setFontSize(16);
    pdf.setTextColor(0, 150, 136);
    pdf.text("POLITIQUE QUALITE", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 4;
    ...
    pdf.setFontSize(12);
    pdf.setTextColor(80, 80, 80);
    const pharmacyName = this.removeAccents(
      data.pharmacyName || "NON RENSEIGNE",
    );
    ...
    const addBoxedSection = (
      ...
      pdf.setTextColor(255, 255, 255);
      pdf.text(
        this.removeAccents(title.toUpperCase()),
        leftMargin + 3,
        yPosition + 4,
      );
      yPosition += 6;
      ...
      const lines = pdf.splitTextToSize(
        contentText,
        pageWidth - leftMargin - rightMargin - 6,
      );
    ...
    // Vision section
    addBoxedSection("VISION", data.vision || "Non renseigne", {
      r: 33,
      g: 150,
      b: 243,
    });
    ...
    // Valeurs section
    addBoxedSection("VALEURS", data.values || "Non renseigne", {
      r: 156,
      g: 39,
      b: 176,
    });
    ...
    // Missions section
    addBoxedSection("MISSIONS", data.missions || "Non renseigne", {
      r: 76,
      g: 175,
      b: 80,
    });
    ...
    addBoxedSection(
      "AXES STRATEGIQUES",
      data.strategicAxes || "Non renseigne",
      { r: 244, g: 67, b: 54 },
    );
    ...
    if (data._signatureImage || data._stampImage) {
      ...
      if (hasSignature && hasStamp) {
        ...
        pdf.text(
          this.removeAccents(data._signatoryName),
          startX + signatureWidth / 2,
          baseY + signatureHeight + 4,
          { align: "center" },
        );
    ...
    pdf.text(
      "PHARMA QMS - Politique Qualite - Page 1/2",
      pageWidth - rightMargin,
      footerY,
      { align: "right" },
    );
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(8);
    const dateCreation = data.dateCreation
    ...
    const creatorNameText = data._creatorName
      ? this.removeAccents(data._creatorName)
      : "__________";
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(8);
    const dateRevision = data.dateRevision
      ? new Date(data.dateRevision).toLocaleDateString("fr-FR")
      : "__________";
    pdf.text(`Date: ${dateRevision}`, rightBoxX + 3, signaturesY + 10);

    // Display verifier name if provided
    const verifierNameText = data._verifierName
      ? this.removeAccents(data._verifierName)
      : "__________";
    ...
    pdf.text(
      "PHARMA QMS - Politique Qualite - Page 2/2",
      pageWidth - rightMargin,
      footerY,
      { align: "right" },
    );
