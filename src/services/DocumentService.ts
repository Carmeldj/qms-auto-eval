...
export class DocumentService {
    ...
    try {
      ...
      if (
        ...
        if (classificationCode && categoryInfo && processInfo) {
          ...
          const descLine1 = `Processus ${processInfo.code}: ${processInfo.name}`;
          pdf.text(descLine1, pageWidth / 2, yPosition, { align: "center" });
          yPosition += 4;

          const descLine2 = `${categoryInfo.name}`;
  ...
  private async generateQualityPolicy(
    ...
    pdf.setFontSize(12);
    pdf.setTextColor(80, 80, 80);
    const pharmacyName = data.pharmacyName || "NON RENSEIGNE";
    ...
    const addBoxedSection = (
      ...
      pdf.setFontSize(9);
      pdf.setTextColor(255, 255, 255);
      pdf.text(title.toUpperCase(), leftMargin + 3, yPosition + 4);
      yPosition += 6;
  ...
  private async generateOrganizationChart(
    ...
    if (salesStaff.length > 0) {
      ...
      if (Object.keys(auxiliaireMap).length > 0 && salesStaff.length < 3) {
        ...
        pdf.text(`AUXILIAIRES: ${initialsText}`, salesX, auxY - 2, {
          align: "center",
        });
      }
