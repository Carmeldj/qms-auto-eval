...
class AdverseEventService {
  async saveReport(report: AdverseEventReport): Promise<void> {
    try {
      const response = await fetch(
        `${SUPABASE_URL}/rest/v1/adverse_event_reports`,
        {
          method: "POST",
          headers: {
  ...
  async sendEmail(
    ...
    try {
      const response = await fetch(
        `${SUPABASE_URL}/functions/v1/send-adverse-event-email`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      await fetch(
        `${SUPABASE_URL}/rest/v1/adverse_event_reports?epid_number=eq.${report.epidNumber}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  private createPDF(report: AdverseEventReport): jsPDF {
    ...
    pdf.setFontSize(11);
    pdf.text(
      "FICHE DE NOTIFICATION DES EFFETS/ÉVÉNEMENTS INDÉSIRABLES",
      pageWidth / 2,
      yPosition,
      { align: "center" }
    );
    ...
    pdf.setFontSize(10);
    pdf.text(
      "OBSERVÉS APRÈS L'UTILISATION DE PRODUIT DE SANTÉ",
      pageWidth / 2,
      yPosition,
      { align: "center" }
    );
    ...
    addText(
      `Date: ${new Date(report.notifier.notificationDate).toLocaleDateString(
        "fr-FR"
      )}`,
      10,
      false,
    ...
    addText(
      `Département: ${report.notifier.department} | ZS: ${report.notifier.zs}`,
      10,
      false,
      1.2
    );
    addText(
      `Commune: ${report.notifier.commune} | FS: ${report.notifier.fs}`,
      10,
      false,
      1.2
    );
    ...
    addText(
      `Qualification: ${report.notifier.qualification.toUpperCase()}`,
      10,
      false,
      1.2
    );
    ...
    addText(
      `Téléphone: ${report.notifier.telephone} | Email: ${report.notifier.email}`,
      10,
      false,
      1.2
    );
    ...
    addText(
      `Nom: ${report.patient.lastName} | Prénom: ${report.patient.firstName}`,
      10
    );
    addText(
      `Sexe: ${report.patient.sex === "M" ? "Masculin" : "Féminin"} | Âge: ${
        report.patient.ageYears || "N/A"
      } ans`,
      10
    );
    if (report.patient.weight || report.patient.height) {
      addText(
        `Poids: ${report.patient.weight || "N/A"} Kg | Taille: ${
          report.patient.height || "N/A"
        } Cm`,
        10
      );
    }
    ...
    if (history.previousReactionSameProduct)
      historyItems.push("Réaction antérieure au même produit");
    if (history.previousReactionOtherProduct)
      historyItems.push("Réaction antérieure à un autre produit");
    ...
    if (ae.localReactionSevere3Days)
      events.push("Réaction locale sévère >3jrs");
    if (ae.localReactionSevereLess3Days)
      events.push("Réaction locale sévère <3jrs");
    if (ae.localReactionBeyondJoint)
      events.push("Réaction locale s'étendant au-delà articulation");
    ...
    if (ae.administrationDate) {
      addText(
        `Date de prise/vaccination: ${new Date(
          ae.administrationDate
    ...
    if (ae.eventDate) {
      addText(
        `Date d'apparition: ${new Date(ae.eventDate).toLocaleDateString(
          "fr-FR"
        )}`,
        10
      );
    ...
    report.suspectProducts.forEach((product, index) => {
      ...
      addText(
        `   Lieu d'achat: ${product.purchaseLocation || "N/A"} | N° Lot: ${
          product.lotNumber || "N/A"
        }`,
        9
      );
      addText(
        `   Voie: ${product.administrationRoute || "N/A"} | Posologie: ${
          product.dosage || "N/A"
        }`,
        9
      );
      ...
      addText(
        `   Dates: ${product.startDate || "N/A"} → ${product.endDate || "N/A"}`,
        9
      );
      yPosition += 2;
    });
    ...
    if (severity.prolongedHospitalization)
      severityItems.push("Prolongation d'hospitalisation");
    if (severity.permanentDisability)
      severityItems.push("Incapacité permanente");
    if (severity.lifeThreatening)
      severityItems.push("Mise en jeu du pronostic vital");
    if (severity.congenitalMalformation)
      severityItems.push("Malformation congénitale");
    ...
    const evolution = [];
    if (severity.recoveryWithoutSequelae)
      evolution.push("Guérison sans séquelle");
    ...
    if (severity.medicationStopped) {
      ...
      if (severity.effectDisappeared)
        addText("  → Effet indésirable: Disparu", 9);
      if (severity.effectDiminished)
        addText("  → Effet indésirable: Diminué", 9);
      if (severity.effectPersisted)
        addText("  → Effet indésirable: Persisté", 9);
    }
    yPosition += 5;
