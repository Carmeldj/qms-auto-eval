...
class ActionPlanService {
  generateActionPlan(
    ...
    pdf.text("LOGO", logoX + logoWidth / 2, logoY + logoHeight / 2, {
      align: "center",
    });
    ...
    const gapsByType = {
      critical: actionPlanData.items.filter(
        (item) => item.gapType === "critical"
      ),
    ...
    ["critical", "major", "minor"].forEach((gapType) => {
      ...
      pdf.setFont("helvetica", "bold");
      pdf.text(
        gapTypeLabels[gapType as keyof typeof gapTypeLabels],
        margin,
        yPosition
      );
      yPosition += 8;
      ...
      headers.forEach((header, i) => {
        ...
        const lineHeight = 4;
        const startY =
          yPosition + (headerHeight - lines.length * lineHeight) / 2 + 3;
    ...
    try {
      const qualitySignature = signatureGenerator.generateSignature(
        "Responsable Qualité",
        250,
        80
      );
    ...
    try {
      const pharmacienSignature = signatureGenerator.generateSignature(
    ...
    pdf.text("PHARMA QMS - Module Inspection", pageWidth - margin, footerY, {
      align: "right",
    });

    const fileName = `Plan_Action_${actionPlanData.pharmacyName.replace(
      /\s+/g,
      "_"
    ...
    generateUploadAndDownloadPDF(pdf, fileName, {
      title: `Plan d'action - ${actionPlanData.pharmacyName}`,
      type: "Plan d'action",
      category: "Inspection",
      description: `Plan d'action corrective et préventive suite à inspection`,
      author: actionPlanData.responsableQualite || "Système",
      version: "1.0",
  ...
  prepareActionPlanItems(report: InspectionReport): ActionPlanItem[] {
    ...
    return nonCompliantAnswers.map((answer) => {
      ...
      return {
        gap: gapDescription,
        gapType,
        establishmentResponse: this.generateDefaultResponse(
          gapDescription,
          gapType
        ),
