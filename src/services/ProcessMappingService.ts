...
class ProcessMappingService {
  generateProcessMapping(): void {
    ...
    // Télécharger le PDF
    const fileName = `Cartographie_Processus_${
      new Date().toISOString().split("T")[0]
    }.pdf`;

    // Upload and download PDF
    generateUploadAndDownloadPDF(pdf, fileName, {
      title: "Cartographie des Processus",
      type: "Cartographie",
      category: "Processus",
      description: "Cartographie complète des processus de la pharmacie",
      author: "Système",
      version: "1.0",
      ...
      console.error("Error uploading process mapping:", error);
    });
  }
  ...
  private generateMappingPage(pdf: jsPDF): void {
    ...
    pdf.text("CARTOGRAPHIE DES PROCESSUS", pageWidth / 2, yPosition, {
      align: "center",
    });
    ...
    pdf.text("Système de Management de la Qualité", pageWidth / 2, yPosition, {
      align: "center",
    });
    ...
    pdf.text(ecoute1, margin + externalBoxWidth / 2, yPosition + 5, {
      align: "center",
    });
    pdf.text(ecoute2, margin + externalBoxWidth / 2, yPosition + 9, {
      align: "center",
    });
    pdf.text(ecoute3, margin + externalBoxWidth / 2, yPosition + 12.5, {
      align: "center",
    });
    ...
    this.drawArrow(
      pdf,
      margin + externalBoxWidth / 2,
      yPosition + externalBoxHeight,
      "down",
      6,
      "blue"
    );
    ...
    pdf.setDrawColor(50, 50, 50);
    pdf.roundedRect(
      pageWidth - margin - externalBoxWidth,
      yPosition,
      externalBoxWidth,
      externalBoxHeight,
      2,
    ...
    pdf.text(
      "Satisfaction client",
      pageWidth - margin - externalBoxWidth / 2,
      yPosition + externalBoxHeight / 2 + 1.5,
      { align: "center" }
    );
    ...
    pdf.text("PROCESSUS DE MANAGEMENT", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 6;

    const mgmtProcesses = processes.filter((p) =>
      ["direction", "qualite"].includes(p.id)
    );
    const boxHeight = 12;

    mgmtProcesses.forEach((process) => {
      this.drawProcessBoxCompact(
        pdf,
        margin + 10,
        yPosition,
        contentWidth - 20,
    ...
    pdf.text("PROCESSUS OPÉRATIONNELS", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 6;
    ...
    pdf.text("PROCESSUS SUPPORT", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 6;
    ...
    pdf.setTextColor(100, 100, 100);
    pdf.text(
      "PHARMA QMS - Cartographie des Processus",
      margin,
      pageHeight - 10
    );
  ...
  private generateProcessDetailPage(pdf: jsPDF, process: any): void {
    ...
    const procedures = documentCategories.filter(
      (doc) => doc.processCode === process.code
    );
    ...
    if (procedures.length > 0) {
      ...
      pdf.setTextColor(100, 100, 100);
      pdf.text(
        "Aucune procédure spécifique identifiée",
        margin + 8,
        yPosition + 4
      );
      yPosition += 7;
  ...
  private getProcessActivities(processId: string): string {
    const activities: Record<string, string> = {
      direction:
        "Définir la politique qualité, fixer les objectifs, allouer les ressources, effectuer les revues de direction, prendre les décisions stratégiques, communiquer avec les parties prenantes.",
      qualite:
        "Planifier les audits internes, gérer les non-conformités, piloter les actions CAPA, mettre à jour la documentation qualité, suivre les indicateurs qualité, assurer la veille réglementaire.",
      approvisionnement:
        "Identifier les besoins, sélectionner les fournisseurs, passer les commandes, réceptionner les produits, contrôler les livraisons, gérer les stocks, optimiser les approvisionnements.",
      dispensation:
        "Accueillir les patients, analyser les ordonnances, délivrer les médicaments, effectuer le conseil pharmaceutique, assurer la traçabilité, gérer les interactions médicamenteuses.",
      retours:
        "Recevoir les réclamations clients, traiter les retours produits, gérer les rappels de lots, analyser les causes, mettre en œuvre les actions correctives, informer les parties concernées.",
      dechets:
        "Trier les produits périmés, séparer les déchets selon leur nature, stocker temporairement les déchets, faire appel aux prestataires de destruction, assurer la traçabilité des destructions.",
      ...
      infrastructures:
        "Maintenir les locaux, entretenir les équipements, surveiller la chaîne du froid, assurer la conformité des installations, planifier les maintenances préventives, gérer les réparations.",
      finances:
        "Gérer la comptabilité, suivre la trésorerie, optimiser les marges, contrôler les stocks financiers, établir les budgets, analyser la rentabilité.",
      "sous-traitance":
        "Identifier les besoins de sous-traitance, qualifier les prestataires, rédiger les cahiers des charges, évaluer les prestations, auditer les prestataires, gérer les contrats.",
    };
  ...
  private drawProcessBoxCompact(
    ...
    pdf.text(lines, x + width / 2, textY, {
      align: "center",
      maxWidth: width - 4,
    });
  }
