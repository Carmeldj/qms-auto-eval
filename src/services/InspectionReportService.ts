...
export class InspectionReportService {
  ...
  public createReport(
    ...
    console.log('InspectionReportService: Creating report with answers:', answers);
  ...
  public calculateSummary(answers: InspectionAnswer[]) {
    console.log('InspectionReportService: Calculating summary for answers:', answers);
  ...
  public calculateRealTimeStats(answers: InspectionAnswer[]) {
    ...
    const stats = {
      ...
      nonCompliant: answeredItems.filter(a => a.status === 'non-compliant').length,
      notApplicable: answeredItems.filter(a => a.status === 'not-applicable').length,
      criticalGaps: answeredItems.filter(a => a.status === 'non-compliant' && a.gapType === 'critical').length,
      majorGaps: answeredItems.filter(a => a.status === 'non-compliant' && a.gapType === 'major').length,
      minorGaps: answeredItems.filter(a => a.status === 'non-compliant' && a.gapType === 'minor').length,
  ...
  public getComplianceRate(answers: InspectionAnswer[]): number {
    const answeredItems = answers.filter(a => a.status !== undefined);
    const compliant = answeredItems.filter(a => a.status === 'compliant').length;
    return answeredItems.length > 0 ? Math.round((compliant / answeredItems.length) * 100) : 0;
  }
  ...
  public async generatePDF(report: InspectionReport): Promise<void> {
    try {
      console.log('InspectionReportService: Starting PDF generation for report:', report);
      ...
      if (!report.answers || report.answers.length === 0) {
        console.warn('InspectionReportService: No answers found, generating empty report');
      }
      ...
      addText(`Pharmacien titulaire: ${report.pharmacistInfo.firstName} ${report.pharmacistInfo.lastName}`, 12, true);
      addText(`N Ordre: ${report.pharmacistInfo.orderNumber}`, 12);
      addText(`Contact: ${report.pharmacistInfo.email} - ${report.pharmacistInfo.phone}`, 12);
      ...
      addText(`Taux de conformite global: ${complianceRate}% - Niveau: ${complianceLevel.level}`, 14, true);
      addText(`Elements evalues: ${realTimeStats.totalItems} sur ${realTimeStats.totalPossibleItems} possibles`, 12);
      
      if (realTimeStats.unanswered > 0) {
        addText(`Elements non evalues: ${realTimeStats.unanswered} (consideres conformes par defaut)`, 12);
      }
      
      addText(`Conformes: ${realTimeStats.compliant} | Non conformes: ${realTimeStats.nonCompliant} | Non applicables: ${realTimeStats.notApplicable}`, 12);
      
      if (realTimeStats.nonCompliant > 0) {
        addText(`Classification des ecarts: Critiques: ${realTimeStats.criticalGaps} | Majeurs: ${realTimeStats.majorGaps} | Mineurs: ${realTimeStats.minorGaps}`, 12);
      }
      ...
      categories.forEach(category => {
        ...
        const categoryComplianceRate = categoryAnsweredCount > 0 ? Math.round((compliantCount / categoryAnsweredCount) * 100) : 0;
        
        addText(`${category}:`, 12, true);
        if (categoryAnsweredCount > 0) {
          addText(`Taux de conformite: ${categoryComplianceRate}% (${compliantCount}/${categoryAnsweredCount} evalues conformes)`, 11);
          addText(`Repartition: Conformes: ${compliantCount} | Non conformes: ${nonCompliantCount} | Non applicables: ${notApplicableCount}`, 10);
          if (unansweredCount > 0) {
            addText(`Elements non evalues: ${unansweredCount} (consideres conformes par defaut)`, 10);
          }
        } else {
          addText(`Aucun element evalue dans cette categorie (${categoryItems.length} elements disponibles)`, 11);
        }
        
        // Detail des elements non conformes
        const nonCompliantItems = categoryAnswers.filter(a => a.status === 'non-compliant');
        if (nonCompliantItems.length > 0) {
          ...
          nonCompliantItems.forEach(answer => {
            ...
            const gapTypeLabel = answer.gapType === 'critical' ? 'CRITIQUE' : 
      ...
      if (recommendations.length > 0) {
        ...
        recommendations.forEach((rec, index) => {
          ...
          addText(`Delai: ${rec.timeframe} | Responsable: ${rec.responsible}`, 9);
          yPosition += 3;
        });
      }
      ...
      if (nonCompliantAnswers.length > 0) {
        ...
        const criticalGaps = nonCompliantAnswers.filter(a => a.gapType === 'critical');
        const majorGaps = nonCompliantAnswers.filter(a => a.gapType === 'major');
        const minorGaps = nonCompliantAnswers.filter(a => a.gapType === 'minor');
        ...
        if (criticalGaps.length > 0) {
          ...
          criticalGaps.forEach((answer, index) => {
            ...
            addText(`Categorie: ${item?.category}${item?.subcategory ? ` - ${item.subcategory}` : ''}`, 10);
            if (answer.comment && answer.comment.trim()) {
              addText(`Observation terrain: ${answer.comment.trim()}`, 10);
            }
            addText(`Impact: Risque significatif pour la securite des patients et la conformite reglementaire`, 10);
        ...
        if (majorGaps.length > 0) {
          ...
          majorGaps.forEach((answer, index) => {
            ...
            addText(`Impact: Peut conduire a la dispensation de produits non conformes`, 10);
        ...
        if (minorGaps.length > 0) {
          ...
          minorGaps.forEach((answer, index) => {
            ...
            addText(`Impact: Ecart aux bonnes pratiques sans impact immediat sur la securite`, 10);
            addText(`Action requise: Integration dans le plan d'amelioration continue (1-3 mois)`, 10);
            yPosition += 3;
          });
        }
        ...
        if (unclassifiedGaps.length > 0) {
          ...
          unclassifiedGaps.forEach((answer, index) => {
            ...
            addText(`Action requise: Evaluation et classification necessaires`, 10);
            yPosition += 3;
          });
        }
      }
      ...
      if (nonCompliantAnswers.length > 0) {
        ...
        addText('Le plan CAPA suivant doit etre mis en oeuvre pour corriger les non-conformites identifiees et prevenir leur recurrence:', 11);
        yPosition += 5;
        ...
        if (immediateActions.length > 0) {
          ...
          immediateActions.forEach((action, index) => {
            ...
            addText(`Criteres de validation: Verification documentaire et controle terrain`, 10);
            addText(`Suivi: Controle quotidien jusqu'a resolution complete`, 10);
            yPosition += 3;
          });
        }

        // Actions correctives urgentes
        const urgentActions = recommendations.filter(r => r.priority === 'urgent');
        if (urgentActions.length > 0) {
          ...
          urgentActions.forEach((action, index) => {
            ...
            addText(`Responsable: ${action.responsible}`, 10);
            addText(`Delai: ${action.timeframe}`, 10);
            addText(`Criteres de validation: Audit de conformite et test d'efficacite`, 10);
            addText(`Suivi: Contrôle hebdomadaire avec rapport d'avancement`, 10);
            yPosition += 3;
          });
        }

        // Actions preventives planifiees
        const plannedActions = recommendations.filter(r => r.priority === 'planned');
        if (plannedActions.length > 0) {
          ...
          plannedActions.forEach((action, index) => {
            ...
            addText(`Responsable: ${action.responsible}`, 10);
            addText(`Delai: ${action.timeframe}`, 10);
            addText(`Criteres de validation: Evaluation d'efficacite et mesure d'amelioration`, 10);
            addText(`Suivi: Contrôle mensuel avec indicateurs de performance`, 10);
            yPosition += 3;
          });
        }
        ...
        addText('Le tableau de suivi suivant doit etre utilise pour monitorer l\'avancement des actions:', 10);
        yPosition += 3;
        
        addText('| Action | Responsable | Delai | Statut | Date de cloture | Efficacite |', 10);
        addText('|--------|-------------|-------|--------|-----------------|------------|', 10);
        
        recommendations.slice(0, 10).forEach(action => {
          addText(`| ${action.title.substring(0, 20)}... | ${action.responsible} | ${action.timeframe} | A faire | ___/___/___ | A evaluer |`, 9);
        });
        ...
        addText('Les indicateurs suivants doivent etre suivis mensuellement:', 10);
        ...
        addText('2. Revue mensuelle des actions preventives et indicateurs', 10);
        addText('3. Validation de l\'efficacite des actions par audit interne', 10);
        addText('4. Cloture des actions apres verification de l\'efficacite', 10);
      ...
      const categoryScores: { category: string; score: number; total: number; percentage: number }[] = [];
      ...
      pdf.rect(margin, yPosition, colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3], cellHeight, 'S');
      ...
      pdf.text('TOTAL', margin + colWidths[0] + colWidths[1] + 5, yPosition + 6);
      pdf.text('TAUX %', margin + colWidths[0] + colWidths[1] + colWidths[2] + 5, yPosition + 6);
      ...
      categoryScores.forEach((cat, index) => {
        ...
        pdf.rect(margin + colWidths[0], yPosition, colWidths[1], cellHeight, 'S');
        pdf.rect(margin + colWidths[0] + colWidths[1], yPosition, colWidths[2], cellHeight, 'S');
        pdf.rect(margin + colWidths[0] + colWidths[1] + colWidths[2], yPosition, colWidths[3], cellHeight, 'S');

        // Contenu
        pdf.setFontSize(9);
        const categoryText = pdf.splitTextToSize(cat.category, colWidths[0] - 4);
        pdf.text(categoryText[0], margin + 2, yPosition + 6);

        pdf.text(cat.score.toString(), margin + colWidths[0] + 8, yPosition + 6);
        pdf.text(cat.total.toString(), margin + colWidths[0] + colWidths[1] + 8, yPosition + 6);

        // Texte du pourcentage sans couleur
        pdf.setFont('helvetica', 'bold');
        pdf.text(`${cat.percentage}%`, margin + colWidths[0] + colWidths[1] + colWidths[2] + 8, yPosition + 6);
        pdf.setFont('helvetica', 'normal');
      ...
      const totalPossible = categoryScores.reduce((sum, cat) => sum + cat.total, 0);
      ...
      pdf.text(totalPossible.toString(), margin + colWidths[0] + colWidths[1] + 8, yPosition + 6);
      pdf.text(`${overallPercentage}%`, margin + colWidths[0] + colWidths[1] + colWidths[2] + 8, yPosition + 6);
      ...
      pdf.setFontSize(8);
      pdf.setTextColor(100, 100, 100);
      pdf.text('Note : Les elements non evalues sont consideres comme conformes dans le calcul du score.', margin, yPosition);
      yPosition += 8;

      // Recommandation professionnelle
      addSection('RECOMMANDATION PROFESSIONNELLE');
      addText('Les resultats de cette auto-inspection montrent qu\'un systeme qualite solide ne peut s\'improviser : il exige methode, suivi et outils adaptes. Pour transformer vos efforts en veritables resultats mesurables, adoptez PharmaQMS, la solution digitale concue pour les officines africaines et alignee sur la norme ISO 9001.', 11, false, 'teal');
      ...
      const safeName = report.pharmacyInfo.name
        ...
        .toLowerCase();
      const fileName = `auto-inspection-${safeName}-${new Date(report.date).toISOString().split('T')[0]}.pdf`;
      pdf.save(fileName);
      
      console.log('InspectionReportService: PDF generated successfully:', fileName);
