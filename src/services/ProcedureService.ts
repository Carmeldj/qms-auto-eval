...
export class ProcedureService {
  ...
  public async generatePDF(procedure: Procedure): Promise<any> {
    try {
      ...
      addText(
        `PROCEDURE DE : ${procedure.info.title.toUpperCase()}`,
        18,
        true,
        "teal",
        "center",
      ...
      const pharmacyDisplayName = procedure.info.pharmacyName
        ? classificationCode
          ? `${procedure.info.pharmacyName} [${classificationCode}]`
      ...
      pdf.setFont("helvetica", "bold");
      pdf.text(
        "Approuvé par: Pharmacien titulaire",
        leftMargin + 3,
        yPosition + 5
      );
      pdf.setFont("helvetica", "normal");
