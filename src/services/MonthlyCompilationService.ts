...
import { supabase } from './TraceabilityRecordService';

export interface MonthlyCompilationData {
  month: number;
  year: number;
  pharmacyName: string;
  registryType: 'equipment-lifecycle' | 'hygiene-tracking' | 'premises-cleaning';
  records: any[];
}
...
class MonthlyCompilationService {
  ...
  async generateMonthlyCompilation(compilationData: MonthlyCompilationData): Promise<void> {
    const doc = new jsPDF();
    const { month, year, pharmacyName, registryType, records } = compilationData;
    ...
    doc.setFontSize(14);
    doc.text(registryTitles[registryType] || 'Registre', 105, 30, { align: 'center' });
    ...
    if (records.length === 0) {
      doc.setFontSize(11);
      doc.text('Aucune entrée pour ce mois', 105, yPosition, { align: 'center' });
    } else {
      doc.setFontSize(11);
      ...
      records.forEach((record, index) => {
        ...
        Object.entries(recordData).forEach(([key, value]) => {
          ...
          const lines = doc.splitTextToSize(`${this.formatFieldName(key)} : ${displayValue}`, 160);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(8);
      doc.text(
        `Compilation générée le ${new Date().toLocaleDateString('fr-FR')} - Page ${i}/${pageCount}`,
        105,
        290,
        { align: 'center' }
      );
    ...
    // Téléchargement
    const fileName = `Compilation_${registryType}_${monthNames[month - 1]}_${year}.pdf`;
    doc.save(fileName);
  }
