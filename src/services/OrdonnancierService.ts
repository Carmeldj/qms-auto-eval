...
class OrdonnancierService {
  ...
  createTrimesterPDF(
    ...
    if (pharmacyInitials) {
      ...
      const processInfo = categoryInfo ? getProcessForCategory('11.02') : undefined;

      if (categoryInfo && processInfo) {
        const classificationCode = generateDocumentCode(pharmacyInitials, processInfo.code, '11.02');
    ...
    if (pharmacistName) {
      try {
        const nameImage = signatureGenerator.generateTextImage(`Pharmacien: ${pharmacistName}`, 120, 15, 9);
        pdf.addImage(nameImage, 'PNG', margin, yPosition - 3, 60, 5, undefined, 'FAST');
        yPosition += 5;
      } catch (error) {
    ...
    if (signatureImage || stampImage) {
      ...
      if (signatureImage) {
        try {
          pdf.addImage(signatureImage, 'PNG', margin, yPosition, signatureWidth, signatureHeight, undefined, 'FAST');
        } catch (error) {
      ...
      if (stampImage) {
        try {
          ...
          pdf.addImage(stampImage, 'PNG', stampX, yPosition, stampSize, stampSize, undefined, 'FAST');
        } catch (error) {
