...
class ProcessReviewService {
  async generatePDF(review: ProcessReview): Promise<void> {
    ...
    pdf.text('COMPTE-RENDU DE REVUE DE PROCESSUS', pageWidth / 2, 15, { align: 'center' });
    pdf.setFontSize(12);
    ...
    addText(`Date de la revue: ${new Date(review.reviewDate).toLocaleDateString('fr-FR')}`, 10, true);
    ...
    if (review.previousActions.length > 0) {
      review.previousActions.forEach((action, index) => {
        ...
        addText(`Responsable: ${action.responsible} | Échéance: ${action.deadline}`, 9);
    ...
    addText(`Note de satisfaction: ${review.beneficiarySatisfaction.rating}/10`, 10);
    addText(`Enquêtes réalisées: ${review.beneficiarySatisfaction.surveysCompleted}`, 10);
    ...
    if (review.indicators.length > 0) {
      review.indicators.forEach((indicator, index) => {
        ...
        addText(`Objectif: ${indicator.target} | Réalisé: ${indicator.actual}`, 9);
    ...
    addText(`Activités planifiées: ${review.activitiesRealization.plannedActivities}`, 9);
    addText(`Activités réalisées: ${review.activitiesRealization.completedActivities}`, 9);
    addText(`Retards ou difficultés: ${review.activitiesRealization.delaysOrIssues}`, 9);

    // 5. Analyse du système documentaire
    addSection('5. ANALYSE DU SYSTÈME DOCUMENTAIRE');
    addText(`Documents à jour: ${review.documentarySystem.documentsUpToDate ? 'Oui' : 'Non'}`, 9);
    addText(`Documents manquants: ${review.documentarySystem.missingDocuments || 'Aucun'}`, 9);
    addText(`Besoins d'amélioration: ${review.documentarySystem.improvementNeeds || 'Aucun'}`, 9);
    ...
    addText(`Ressources financières: ${review.resources.financialResources}`, 9);
    ...
    if (review.risksAndOpportunities.identifiedRisks.length > 0) {
      review.risksAndOpportunities.identifiedRisks.forEach((risk, index) => {
        ...
        addText(`Gravité: ${this.getSeverityLabel(risk.severity)} | Probabilité: ${this.getProbabilityLabel(risk.probability)}`, 9);
    ...
    if (review.risksAndOpportunities.identifiedOpportunities.length > 0) {
      review.risksAndOpportunities.identifiedOpportunities.forEach((opp, index) => {
    ...
    addText(`Actions non démarrées: ${review.actionsBilan.notStartedActions}`, 9);
    ...
    addText(`Évaluation: ${this.getEfficiencyLabel(review.overallEfficiency.rating)}`, 10, true);
    ...
    pdf.text('Ce document constitue le compte-rendu officiel de la revue de processus.', margin, yPosition);
    yPosition += 4;
    pdf.text(`Signé par: ${review.pilotName}, Pilote du processus`, margin, yPosition);
    yPosition += 4;
    pdf.text(`Date: ${new Date(review.reviewDate).toLocaleDateString('fr-FR')}`, margin, yPosition);

    const fileName = `revue-processus-${review.processName.replace(/\s+/g, '-')}-${review.reviewYear}.pdf`;
    pdf.save(fileName);
  }
